...
(echo %FILE_PATH% & echo %MATRIX_SIZE%) | java -cp build\classes opsys_project.MatrixMultiplier_SingleThread > temp_output.txt

for /f "tokens=4" %%A in ('findstr /C:"Time taken" temp_output.txt') do (
...
for %%T in (%THREAD_COUNTS%) do (
    ...
    (echo %FILE_PATH% & echo %MATRIX_SIZE% & echo %%T) | java -cp build\classes opsys_project.MatrixMultiplier_MultiThread > temp_output.txt
